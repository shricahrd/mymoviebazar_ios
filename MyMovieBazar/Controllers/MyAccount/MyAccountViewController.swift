...
class MyAccountViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.row == 2 {
            ...
            if let myPlansController = storyboard.instantiateViewController(withIdentifier: "MyPlansController") as? MyPlansController {
              navigationController?.pushViewController(myPlansController, animated: true)
            }
        }
        if indexPath.row == 3 {
            let storyboard: UIStoryboard = UIStoryboard(name: "Main", bundle: Bundle.main)
            if let myPlansController = storyboard.instantiateViewController(withIdentifier: "MyAddressDetail") as? MyAddressDetail {
              navigationController?.pushViewController(myPlansController, animated: true)
            }
        }
